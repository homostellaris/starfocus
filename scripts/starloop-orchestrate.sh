...
if [ "$active_count" -lt "$MAX_CONCURRENCY" ]; then
  ...
  openclaw agent \
    --message "Run /starloop $TODOS_DIR $STAR_ROLE and send the result via: openclaw message send --channel telegram --target $TELEGRAM_TARGET --message '[message]'. When the user replies with go, a number, or a task name: spawn a Claude Code ACP session by running: acpx claude sessions new --name [todo-filename-no-ext] (cwd: $ACPX_WORKSPACE). Then send the task via: acpx claude -s [todo-filename-no-ext] \"Read $TODOS_DIR/[chosen-filename] and execute the task. If you need input, send: openclaw message send --channel telegram --target $TELEGRAM_TARGET --message YOUR_QUESTION and pause.\". Do NOT discuss the task or ask any questions — just spawn, then confirm to the user via Telegram: '🚀 Started session [todo-filename-no-ext]. I will update you when done or if Claude needs input.'"
else
