...
The webhook secret lives in the bridge env file (only used for verifying incoming webhooks):
...
The app credentials live separately (used by `github-app-token.ts` when Claude Code needs to act on GitHub):

```bash
# ~/.config/github-app.env
GITHUB_APP_ID=<your-app-id>
GITHUB_APP_INSTALLATION_ID=<your-installation-id>
GITHUB_APP_PRIVATE_KEY_PATH=/home/<user>/.config/github-app.pem
GITHUB_BOT_GIT_NAME=<app-name>[bot]
GITHUB_BOT_GIT_EMAIL=<app-user-id>+<app-name>[bot]@users.noreply.github.com
```

Find `<app-user-id>` by calling `GET /users/<app-name>[bot]` after installing the app:
...
When Claude Code posts a PR comment or creates a PR, it should authenticate as the bot rather than your personal account. The `ghb` wrapper at `~/.local/bin/ghb` handles this by auto-generating a fresh installation token for every `gh` call:
...
```bash
# Create the wrapper (replace the path with your starfocus repo location)
cat > ~/.local/bin/ghb << 'EOF'
#!/usr/bin/env bash
GH_TOKEN=$(bun run /path/to/starfocus/agent/scripts/github-app-token.ts) exec gh "$@"
EOF
chmod +x ~/.local/bin/ghb
```

For git commits, the bot identity is stored in `~/.config/github-app.env` (the `GITHUB_BOT_GIT_NAME` and `GITHUB_BOT_GIT_EMAIL` fields). Autonomous sessions set it via `git config` before committing — see `agent/rules/autonomous.md`.
